...
@endif --}}
<div>¿Usted has sido notificado como un caso de ALERTA COVID, caso SOSPECHOSO, caso CONFIRMADO o caso PROBABLE?  : <span style="color: #3F7FBF">{{ $alerta_covid }}</span> </div>

@if($sintomas)
    <div>En los últimos 15 días ha tenido o tiene los siguientes síntomas:  <span style="color: #3F7FBF">{{ $sintomas }} </span> </div>
@endif
<div>¿Ha tenido en su último descanso algún contacto con personas que han presentado algunos de los síntomas relacionado a Covid-19 o se les ha practicado un examen de PCR o Antígeno? : <span style="color: #3F7FBF">{{ $contacto_pcr }}</span> </div>
<div>¿Algún miembro de su grupo familiar durante los últimos 7 días, ha tenido contacto con personas que han presentado síntomas relacionados a Covid-19 o se les ha practicado un examen de PCR?: <span style="color: #3F7FBF">{{ $miembro_familia_pcr }}</span> </div>
